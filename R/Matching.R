...
Match <- function (Y = NULL, Tr, X, Z = X, V = rep(1, length(Y)), estimand = "ATT", 
  ...
  if (version == "fast" | version == "standard") {
    if (!is.null(match.out)) {
      ...
      ret <- RmatchLoop(Y = Y, Tr = Tr, X = X, Z = Z, V = V, 
                        ...
                        weight = weights, SAMPLE = sample, ccc = ccc, 
                        cdd = cdd, ecaliper = ecaliper, exact = exact, 
...
#      } #end of exact
    ...
    Weight.matrix.global <<- Weight.matrix
